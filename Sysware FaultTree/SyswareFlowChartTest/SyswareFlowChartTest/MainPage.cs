  ...
  new ElementTemplate[]
    ...
    new ShapeDecoration[]
	{
        ...
        private void diagramView1_DoubleClick(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                if (Snode.ItemType == "顶")//顶
                {
                    ...
                    using (SetTopNode dlg = new SetTopNode(Snode, diagram1))
                    {
                        if (dlg.ShowDialog() == System.Windows.Forms.DialogResult.OK)
                        {
        ...
        private void ToolStripMenuItemDelete_Click(object sender, EventArgs e)
        {
            ...
            DeleteParrentContianInfo();
            while (stacks.Count > 0)
            {
                this.diagram1.Items.Remove(stacks.Pop());
            }
        }
        private void DeleteParrentContianInfo()
        {
            SyswareNode current = (SyswareNode)this.diagram1.ActiveItem;
            foreach (var link in this.diagram1.Links.Where(w => w.Destination == current))
            {
                SyswareNode sn = (SyswareNode)link.Origin;
                NodeInfos ni = sn.Tag as NodeInfos;
                NodeInfos cuni = current.Tag as NodeInfos;
                for (int i = 0; i < ni.ContainsNodes.Count(); i++)
                {
                    if (ni.ContainsNodes[i].Code == cuni.Code)
                    {
                        ni.ContainsNodes.RemoveAt(i); break;
                    }
                }
            }
        }
