...
namespace tomoral.CRM.Data
{
    ...
    public class hr_post : IHttpHandler
    {
        ...
        public void ProcessRequest(HttpContext context)
        {
            ...
            tomoral.Model.hr_post model = new tomoral.Model.hr_post();
            ...
            if (request["Action"] == "grid")
            {
                ...
                {
                    empstuats = tomoral.Common.PageValidate.IsNumber(request["empstatus"]) ? int.Parse(request["empstatus"]) : 0;
                }
            ...
            if (request["Action"] == "save")
            {
                ...
                //emp
                int empid = tomoral.Common.PageValidate.IsNumber(request["T_emp_val"]) ? int.Parse(request["T_emp_val"]) : -1;
                model.emp_id = empid;
                model.emp_name = PageValidate.InputText(request["T_emp"], 250);
                //note
                model.note = PageValidate.InputText(request["T_descript"], 4000);

                tomoral.Model.hr_employee modelemp = new tomoral.Model.hr_employee();
                //更新员工岗位
            ...
            if (request["Action"] == "form")
            {
                ...
                tomoral.BLL.hr_post hp = new tomoral.BLL.hr_post();
            ...
            if (request["Action"] == "postemp")
            {
                ...
                {
                    Eid = tomoral.Common.PageValidate.IsNumber(empid) ? int.Parse(empid) : -1;
                }
                tomoral.Model.hr_employee modelemp = new tomoral.Model.hr_employee();
                model.emp_id = Eid;
                modelemp.ID = Eid;
